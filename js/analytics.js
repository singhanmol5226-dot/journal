/**
 * analytics.js — All analytics calculations for Trade Journal
 */
...
function calcPnL(trade) {
  ...
  const raw = isLong ? (exit - entry) * qty : (entry - exit) * qty;
...
function calcRR(trade) {
  ...
  if (!sl || !target || sl === entry) return null;
...
function computeAnalytics(trades) {
  ...
  // Enrich trades with computed pnl/outcome
  ...
  // Streaks
  const sorted = [...enriched].sort((a, b) => new Date(a.entryDate + ' ' + (a.entryTime || '00:00')) - new Date(b.entryDate + ' ' + (b.entryTime || '00:00')));
  ...
  const dowMap = {};
  for (const t of enriched) {
    const d = t.entryDate ? new Date(t.entryDate).getDay() : -1;
    if (d < 0) continue;
