/**
 * ai.js — AI Analysis for MT5-enabled Trade Journal
 * OpenAI key fetched from backend settings API
 */

const OVERTRADING_THRESHOLD = 5;
const MAX_LOSS_BAD_THRESHOLD = -1000;
    ...
    <div class="ai-section-grid">
      ...
      <div class="ai-panel">
        ...
        <div class="openai-key-section">
          <label style="font-size:13px;color:var(--text-secondary);display:block;margin-bottom:6px;">OpenAI API Key (saved in backend settings)</label>
          ...
          <p style="font-size:12px;color:var(--text-secondary);margin-top:6px;">
            Your key is stored in the backend settings database and never sent anywhere except OpenAI's API.
          </p>
        </div>
...
function generateBuiltinInsights(trades) {
  ...
  // Best/Worst day of week — use open_time
  ...
  for (const t of enriched) {
    ...
    if (!ot) continue;
    let dow;
    try { dow = new Date(ot.slice(0, 10) + 'T12:00:00').getDay(); } catch { continue; }
  ...
  // Emotion correlation — use emotion_before field
  const emotionStats = {};
  for (const t of enriched) {
    const e = t.emotion_before || t.emotionBefore || '';
    if (!e) continue;
  ...
  if (emotionArr.length) {
    ...
    insights.push({ icon: '😊', title: `Emotion: "${bestEmo.name}" → Most Profitable`, body: `${bestEmo.wr}% win rate across ${bestEmo.total} trades.`, type: 'good' });
    if (worstEmo.name !== bestEmo.name) {
      insights.push({ icon: '😟', title: `Emotion: "${worstEmo.name}" → Hurts Performance`, body: `Only ${worstEmo.wr}% win rate across ${worstEmo.total} trades.`, type: 'bad' });
    }
  }

  // Overtrading — use open_time date
  const tradesPerDay = {};
  for (const t of enriched) {
  ...
  if (overtradeDays > 0) {
    insights.push({ icon: '🔥', title: 'Overtrading Detected', body: `You had ${overtradeDays} day(s) with ${OVERTRADING_THRESHOLD}+ trades. Overtrading often leads to emotional decisions.`, type: 'bad' });
  }
  ...
  for (let i = 2; i < sorted.length; i++) {
    const prev2 = sorted[i - 2], prev1 = sorted[i - 1], curr = sorted[i];
    if (prev2.pnl < 0 && prev1.pnl < 0) {
      const prevDate1 = (prev1.open_time || '').slice(0, 10);
      const prevDate2 = (prev2.open_time || '').slice(0, 10);
      const currDate = (curr.open_time || '').slice(0, 10);
      const sameDayBefore = prevDate1 === prevDate2 || prevDate1 === currDate;
      if (sameDayBefore) revengeCount++;
    }
  }
  if (revengeCount > 0) {
    insights.push({ icon: '😤', title: 'Possible Revenge Trading', body: `Detected ${revengeCount} instance(s) of trading after 2 consecutive losses on the same day.`, type: 'bad' });
  }

  // Risk analysis — use stop_loss / take_profit
  const rrTrades = enriched.filter(t => t.stop_loss && t.take_profit && t.open_price);
  if (rrTrades.length) {
    const avgRR = rrTrades.reduce((s, t) => {
      const ep = parseFloat(t.open_price) || 0;
      const sl = parseFloat(t.stop_loss) || 0;
      const tgt = parseFloat(t.take_profit) || 0;
      const risk = Math.abs(ep - sl);
      const reward = Math.abs(tgt - ep);
      return s + (risk > 0 ? reward / risk : 0);
    }, 0) / rrTrades.length;
    insights.push({ icon: '⚖️', title: 'Risk-Reward Analysis', body: `Average planned R:R is ${avgRR.toFixed(2)}:1 across ${rrTrades.length} trades with stop/target. ${avgRR >= 2 ? 'Excellent!' : avgRR >= 1 ? 'Good, aim for 2:1+.' : 'Consider improving your R:R.'}`, type: avgRR >= 2 ? 'good' : avgRR >= 1 ? 'neutral' : 'bad' });

    const maxLoss = Math.min(...enriched.map(t => t.pnl));
    insights.push({ icon: '🛡️', title: 'Max Single Loss', body: `Your worst trade lost ${Math.abs(maxLoss).toFixed(2)}. Ensure this aligns with your risk rules.`, type: maxLoss < MAX_LOSS_BAD_THRESHOLD ? 'bad' : 'neutral' });
  }

  // Morning vs Afternoon — use open_time hours
  const tradesWithTime = enriched.filter(t => (t.open_time || '').length >= 13);
  if (tradesWithTime.length >= 4) {
    const morningTrades = tradesWithTime.filter(t => parseInt((t.open_time || '').slice(11, 13)) < 12);
    const afternoonTrades = tradesWithTime.filter(t => parseInt((t.open_time || '').slice(11, 13)) >= 12);
...
function downloadAIReport() {
  ...
  report += `Wins: ${wins} | Losses: ${enriched.filter(t => t.pnl < 0).length} | Win Rate: ${winRate}%\n`;
...
async function analyzeWithOpenAIBtn() {
  ...
  await sendMessage(key, question);
}
...
async function sendMessage(apiKey, question) {
